...
bool Article::ajouter()
{
    ...
    query.prepare("INSERT INTO ARTICLE (titre, contenu, domaine, Etat, Date_creation, ID_user) "
                  "VALUES (:titre, :contenu, :domaine, :etat, :date, :idUser)");
...
bool Article::modifier()
{
    ...
    query.prepare("UPDATE ARTICLE SET titre = :titre, contenu = :contenu, domaine = :domaine, "
                  "Etat = :etat, Date_creation = :date, ID_user = :idUser "
                  "WHERE ID_article = :id");
