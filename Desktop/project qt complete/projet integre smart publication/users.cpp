...
#include "FaceCaptureDialog.h"  // ← NOUVEAU : Dialog de capture faciale
...
void Users::setupConnections()
{
    ...
    // === NOUVEAU : Bouton Capture Face (Arduino 3) ===
    connect(ui->captureFaceButton, &QPushButton::clicked, this, &Users::onCaptureFaceClicked);
}
...
{
    // Check the current echo mode of the password input
...
// ═══════════════════════════════════════════════════════════════════════════
//  NOUVEAU : Capture de visage pour reconnaissance faciale (Arduino 3)
// ═══════════════════════════════════════════════════════════════════════════
void Users::onCaptureFaceClicked()
{
    QString email = ui->emailInput->text().trimmed();
    
    if (email.isEmpty()) {
        QMessageBox::warning(this, "Email Requis", 
            "Veuillez d'abord entrer l'adresse email de l'utilisateur avant de capturer son visage.");
        ui->emailInput->setFocus();
        return;
    }
    ...
    if (!emailRegex.match(email).hasMatch()) {
        QMessageBox::warning(this, "Email Invalide", 
            "Veuillez entrer une adresse email valide avant de capturer le visage.");
        ui->emailInput->setFocus();
        return;
    }
    
    qDebug() << "[Users] Démarrage de la capture faciale pour:" << email;
    
    FaceCaptureDialog captureDialog(email, this);
    
    if (captureDialog.exec() == QDialog::Accepted) {
        QString savedDir = captureDialog.savedDirectory();
        
        if (!savedDir.isEmpty()) {
            ui->faceStatusLabel->setText("✅ Visage enregistré avec succès !");
            ui->faceStatusLabel->setStyleSheet("font-size: 13px; color: #10B981; font-weight: 600;");
            
            QMessageBox::information(this, "Succès", 
                QString("10 échantillons de visage capturés avec succès !\n\n"
                        "Dossier : %1\n\n"
                        "L'utilisateur pourra maintenant se connecter par reconnaissance faciale.")
                    .arg(savedDir));
            
            qDebug() << "[Users] Capture faciale réussie. Dossier:" << savedDir;
        } else {
            ui->faceStatusLabel->setText("❌ Échec de l'enregistrement");
            ui->faceStatusLabel->setStyleSheet("font-size: 13px; color: #EF4444;");
            QMessageBox::warning(this, "Échec", "Impossible d'enregistrer les échantillons de visage.");
        }
    } else {
        ui->faceStatusLabel->setText("⏳ Aucun visage capturé");
        ui->faceStatusLabel->setStyleSheet("font-size: 12px; color: #64748B;");
        qDebug() << "[Users] Capture faciale annulée";
    }
}
