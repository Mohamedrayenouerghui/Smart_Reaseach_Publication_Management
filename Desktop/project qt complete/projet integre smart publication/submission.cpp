...
void Submission::setArduinoManager(ArduinoManager *manager)
{
...
void Submission::setupMenus()
{
    ...
    connect(ui->sortTypeCombo, QOverload<int>::of(&QComboBox::currentIndexChanged), this, [this](int) {
...
void Submission::setupConnections()
{
    ...
    connect(ui->arduinoBtn, &QPushButton::clicked, this, &Submission::onArduinoClicked);
...
void Submission::addButtonsToRow(int row, int id)
{
    ...
    connect(decisionBtn, &QPushButton::clicked, this, [this, id, row]() {
        ...
        QString articleTitle = ui->submissionTable->item(row, 0)->text();
        
        if (arduinoManager && arduinoManager->isArduinoConnected()) {
            // Ouvrir le dialog de décision Arduino
            DecisionDialog *dialog = new DecisionDialog(id, articleTitle, arduinoManager, this);
            
            if (dialog->exec() == QDialog::Accepted && dialog->wasDecisionMade()) {
                // La décision a été prise via Arduino
            ...
            // Arduino non connecté - utiliser le menu classique
            QMenu menu;
            menu.setStyleSheet(
    ...
    if (msgBox.clickedButton() == notifyBtn) {
        ...
        if (file.open(QIODevice::Append | QIODevice::Text)) {
            ...
            successBox.setText("🔔 Notification envoyée avec succès!\n\n"
                              "Le relecteur " + relecteur.split(" - ").first() + " sera notifié lors de sa prochaine connexion.\n\n"
                              "📄 Article: " + article + "\n"
                              "📅 Date limite: " + dateLimite.toString("dd/MM/yyyy"));
...
void Submission::onAutoAssignReviewer()
{
    ...
    // Liste fixe des relecteurs avec leur domaine et mots-clés
    ...
    // Mots-clés pour chaque domaine (pour améliorer la correspondance)
    QMap<QString, QStringList> keywords = {
        {"Intelligence Artificielle", {"intelligence", "artificielle", "ia", "ai", "machine learning", "deep learning", "neural"}},
        {"Cybersécurité", {"cyber", "sécurité", "securite", "security", "cryptographie", "réseau", "network"}},
        {"Data Science", {"data", "science", "données", "donnees", "analyse", "statistique", "big data"}},
        {"Génie Logiciel", {"génie", "genie", "logiciel", "software", "engineering", "développement", "developpement", "programmation"}}
    };
    ...
    if (q.exec() && q.next())
        articleDomain = q.value(0).toString().toLower();

    // Trouver le relecteur dont le domaine correspond
    QString assignedReviewer;
    QString expertise;
    
    // Essayer de trouver une correspondance avec les mots-clés
    for (auto it = relecteurs.begin(); it != relecteurs.end(); ++it) {
        QString reviewerDomain = it.value();
        QStringList domainKeywords = keywords[reviewerDomain];
        
        // Vérifier si le domaine de l'article contient un des mots-clés
        for (const QString &keyword : domainKeywords) {
            if (articleDomain.contains(keyword, Qt::CaseInsensitive)) {
                assignedReviewer = it.key() + " - " + reviewerDomain;
                expertise = reviewerDomain;
                break;
            }
        }
    ...
    // Fallback: choisir aléatoirement parmi les relecteurs
    if (assignedReviewer.isEmpty()) {
        QStringList allReviewers;
        for (auto it = relecteurs.begin(); it != relecteurs.end(); ++it) {
            allReviewers << (it.key() + " - " + it.value());
        }
        int randomIndex = QRandomGenerator::global()->bounded(allReviewers.size());
        assignedReviewer = allReviewers[randomIndex];
        expertise = assignedReviewer.split(" - ").last();
    }
...
void Submission::onArduinoClicked()
{
    // Obtenir le MainWindow parent
    ...
    if (parent) {
        // Appeler la fonction showArduinoInterface du MainWindow
        QMetaObject::invokeMethod(parent, "showArduinoInterface");
    }
}
